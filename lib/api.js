...
// High-performance SWR Memory & Storage Cache for Schools, Classes & Categories
...
export function getCachedCategories() {
  if (memoryCategoriesCache && Array.isArray(memoryCategoriesCache) && memoryCategoriesCache.length > 0) {
    return memoryCategoriesCache;
  }
  if (typeof window !== 'undefined') {
    try {
      const stored = sessionStorage.getItem('bsmart_categories_cache_v2');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) {
...
export async function getLiveCategories(forceFresh = false) {
  if (!forceFresh && memoryCategoriesCache && Date.now() - lastCategoriesCacheTime < MASTERS_CACHE_TTL_MS) {
    return memoryCategoriesCache;
  }

  try {
    const targetUrl = typeof window !== 'undefined'
      ? `/api/categories${forceFresh ? '?fresh=1' : ''}`
    ...
    if (!res.ok && targetUrl !== `${API_BASE_URL}/api/categories` && API_BASE_URL) {
      res = await fetch(`${API_BASE_URL}/api/categories`, { cache: 'no-store' });
    }

    if (res.ok) {
      const data = await res.json();
      if (data.success && Array.isArray(data.categories)) {
        const cleanCategories = data.categories.filter((c) => c && typeof c === 'string' && c.trim().length > 0);
        memoryCategoriesCache = cleanCategories;
        lastCategoriesCacheTime = Date.now();
        if (typeof window !== 'undefined') {
          try {
            sessionStorage.setItem('bsmart_categories_cache_v2', JSON.stringify(cleanCategories));
          } catch (e) {}
        }
    ...
    console.warn('Live Categories API notice:', error.message);
  }

  const cached = getCachedCategories();
  if (cached) return cached;
  return ['Shirt', 'Pant', 'Sweater', 'Tie', 'Belt', 'Coat/Blazer', 'Accessories'];
}
