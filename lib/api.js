...
export function getCachedSchools() {
  ...
  if (typeof window !== 'undefined') {
    try {
      const stored = sessionStorage.getItem('bsmart_schools_cache_v1');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) {
...
export async function getLiveSchools(forceFresh = false) {
  if (!forceFresh && memorySchoolsCache && Date.now() - lastSchoolsCacheTime < CACHE_TTL_MS) {
    return memorySchoolsCache;
  }

  try {
    const targetUrl = typeof window !== 'undefined' ? '/api/schools' : `${API_BASE_URL}/api/schools`;
    let res = await fetch(targetUrl, {
      cache: 'default',
      next: { revalidate: 30 },
    });

    if (!res.ok && targetUrl !== `${API_BASE_URL}/api/schools`) {
    ...
    if (res.ok) {
      ...
      if (data.success && Array.isArray(data.schools)) {
        ...
        if (typeof window !== 'undefined') {
          try {
            sessionStorage.setItem('bsmart_schools_cache_v1', JSON.stringify(cleanSchools));
          } catch (e) {}
        }
        return cleanSchools;
      }
...
export function getCachedClasses() {
  ...
  if (typeof window !== 'undefined') {
    try {
      const stored = sessionStorage.getItem('bsmart_classes_cache_v1');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) {
...
export async function getLiveClasses(forceFresh = false) {
  if (!forceFresh && memoryClassesCache && Date.now() - lastClassesCacheTime < CACHE_TTL_MS) {
    return memoryClassesCache;
  }

  try {
    const targetUrl = typeof window !== 'undefined' ? '/api/classes' : `${API_BASE_URL}/api/classes`;
    let res = await fetch(targetUrl, {
      cache: 'default',
      next: { revalidate: 30 },
    });

    if (!res.ok && targetUrl !== `${API_BASE_URL}/api/classes`) {
    ...
    if (res.ok) {
      ...
      if (data.success && Array.isArray(data.classes)) {
        ...
        if (typeof window !== 'undefined') {
          try {
            sessionStorage.setItem('bsmart_classes_cache_v1', JSON.stringify(cleanClasses));
          } catch (e) {}
        }
        return cleanClasses;
      }
